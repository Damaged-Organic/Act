...
def canonical_fieldname(db_field):
    """
    All "description_en", "description_fr", etc. field names
...
def mandatory_language():
    ...
    return getattr(
        settings, 'TRANSMETA_MANDATORY_LANGUAGE', fallback_language()
    )
...
def get_all_translatable_fields(
    model, model_trans_fields=None, column_in_current_table=False
):
    """
...
def default_value(field):
    ...
    def default_value_func(self):
        def attname(x):
            return get_real_fieldname(field, x)
...
class TransMeta(models.base.ModelBase):
    def __new__(cls, name, bases, attrs):
        ...
        if 'Meta' in attrs and hasattr(attrs['Meta'], 'translate'):
            ...
            new_class = super(TransMeta, cls).__new__(
